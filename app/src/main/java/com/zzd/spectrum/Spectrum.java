...
public class Spectrum extends Activity {
	...
	class SpectrumView extends View {
		...
		protected void onDraw(Canvas canvas) {
			...
			if (specmode == 3) { // waveform
				...
				BASS.BASS_ChannelGetInfo(chan, ci);
				ByteBuffer bbuf = ByteBuffer.allocateDirect(ci.chans * width * 4); // allocate buffer for data
				bbuf.order(null); // little-endian byte order
				BASS.BASS_ChannelGetData(chan, bbuf, ci.chans * width * 4); // get the sample data
				float[] pcm = new float[ci.chans * width]; // allocate a "float" array for the sample data
				bbuf.asFloatBuffer().get(pcm); // get the data from the buffer into the array
				for (int c = 0; c < ci.chans; c++) {
					for (int x = 0, y = 0; x < width; x++) {
						int v = (int) ((1 - pcm[x * ci.chans + c]) * height / 2); // invert and scale to fit display
						if (v < 0) v = 0;
				...
				.setItems(filelist, new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						...
						if (sel.isDirectory()) {
							...
							handler.postDelayed(timer, 1000 / specview.refreshrate);
						}
					}
				})
				.show();
	...
	Runnable timer = new Runnable() {
		public void run() {
			specview.invalidate();
			handler.postDelayed(this, 1000 / specview.refreshrate);
		}
	};
